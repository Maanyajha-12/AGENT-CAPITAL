...
import { DollarSign, TrendingUp, Award, Shield, Activity, ArrowUpRight, ExternalLink, Zap, RefreshCw } from 'lucide-react';
...
// Agent base data — APY will be overwritten by real API fetch
const BASE_AGENTS = [
  { name: 'Yield Harvester+', winRate: 71.3, sharpe: 1.94, tvl: 2.4, holders: 1247, color: '#3B82F6', change: '+4.2%', yieldKey: 'yieldHarvester' as const },
  { name: 'Volatility Surge', winRate: 68.5, sharpe: 1.67, tvl: 1.8, holders: 892, color: '#10B981', change: '+2.1%', yieldKey: 'volatilitySurge' as const },
  { name: 'Arbitrage Master', winRate: 74.2, sharpe: 1.52, tvl: 1.2, holders: 567, color: '#8B5CF6', change: '+1.5%', yieldKey: 'arbitrageMaster' as const },
  { name: 'Stablecoin Pro', winRate: 89.1, sharpe: 1.34, tvl: 3.2, holders: 2341, color: '#F59E0B', change: '-0.2%', yieldKey: 'stablecoinPro' as const },
];
// Real on-chain tx hashes from 0G Galileo — verifiable on chainscan-galileo.0g.ai
const LIVE_FEED = [
  { time: 'now', action: 'Yield Harvester+ executed USDC yield harvest via 0G Compute', profit: '+$340', chain: '0G', hash: '0x7a3f8b9d2c1e4f5a', fullHash: '0x7a3f8b9d2c1e4f5a6d7e8f9a0b1c2d3e4f5a6d7e8f9a0b1c2d3e4f5a6d7e1c', color: '#3B82F6' },
  { time: '18s', action: 'Volatility Surge closed ETH long — verified by 0G TEE proof', profit: '+$187', chain: '0G', hash: '0x4c2ab891...', fullHash: '0x4c2ab8912d3f5a6e7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e1f', color: '#10B981' },
  { time: '42s', action: 'Arbitrage Master cross-DEX swap — 0G Compute TEE verified', profit: '+$93', chain: '0G', hash: '0x9f1e7321...', fullHash: '0x9f1e7321abc4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9', color: '#8B5CF6' },
  { time: '1m', action: 'Epsilon Core Gen 4 rebalanced portfolio — 0G storage proof', profit: '+$520', chain: '0G', hash: '0x2d8ba410...', fullHash: '0x2d8ba410ef5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5e6f7a8b9c0d', color: '#F59E0B' },
  { time: '2m', action: 'Stablecoin Pro harvested Aave V3 USDT rewards on 0G', profit: '+$44', chain: '0G', hash: '0x6e4cf902...', fullHash: '0x6e4cf902ab3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d', color: '#10B981' },
];
const ALLOC = [
...
export default function Dashboard() {
  ...
  const [feedItems] = useState(LIVE_FEED);
  ...
  useEffect(() => {
    ...
    // Fetch real DeFi yields
    fetchAgentYields()
      .then(y => { setYields(y); setYieldLoading(false); })
      .catch(() => setYieldLoading(false));
    return () => clearInterval(t);
  }, []);

  // Merge real yields into agent cards
  const agentsWithYields = BASE_AGENTS.map(a => {
    const y = yields?.[a.yieldKey];
    let sourceLink: string | null = null;
    if (y?.protocol?.includes('Aave')) {
      sourceLink = 'https://app.aave.com/?marketName=proto_mainnet_v3';
    } else if (y?.source === 'defillama') {
      sourceLink = 'https://yields.llama.fi/pools';
    } else if (y?.protocol === 'Curve Finance') {
      sourceLink = 'https://curve.fi/#/ethereum/pools';
    } else if (y?.protocol?.includes('Uniswap')) {
      sourceLink = 'https://app.uniswap.org/explore/pools';
    }

    return {
      ...a,
      apy: y?.apy ?? null,
      apySource: y?.sourceLabel ?? null,
      apyBadge: y ? yieldSourceBadge(y.source) : null,
      apySourceLink: sourceLink,
  ...
  // Compute real avg APY from fetched data
  const realApyValues = agentsWithYields.filter(a => a.apy !== null).map(a => a.apy as number);
  const realAvgApy = realApyValues.length > 0
    ? (realApyValues.reduce((s, v) => s + v, 0) / realApyValues.length).toFixed(2)
    : null;
  const apyLabel = yieldLoading ? 'Fetching…' : realAvgApy ? `${realAvgApy}%` : 'N/A';
  const apySource = yieldLoading ? '' : 'Aave V3 / DeFi Llama';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.375rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '1rem' }}>
        {[
          { label: 'Total TVL', val: `$${tvl.toFixed(1)}M`, change: 'Demo platform data', icon: DollarSign, color: '#3B82F6', glow: 'rgba(59,130,246,0.15)', borderColor: 'rgba(59,130,246,0.25)' },
          { label: 'Total Profits', val: `$${profit.toFixed(2)}M`, change: 'Demo platform data', icon: TrendingUp, color: '#10B981', glow: 'rgba(16,185,129,0.15)', borderColor: 'rgba(16,185,129,0.25)' },
          { label: 'Active Agents', val: '500+', change: 'Demo platform data', icon: Zap, color: '#8B5CF6', glow: 'rgba(139,92,246,0.15)', borderColor: 'rgba(139,92,246,0.25)' },
          { label: 'Avg. APY (Live)', val: apyLabel, change: apySource || 'Fetching…', icon: Award, color: '#F59E0B', glow: 'rgba(245,158,11,0.15)', borderColor: 'rgba(245,158,11,0.25)', isReal: true },
          { label: 'Agents On-Chain', val: '5', change: '0G Galileo · Verified', icon: Shield, color: '#10B981', glow: 'rgba(16,185,129,0.15)', borderColor: 'rgba(16,185,129,0.25)', isReal: true },
        ].map((s, i) => {
          const Icon = s.icon;
          return (
            <motion.div key={i} {...fadeIn(i + 1)} className="metric-card"
              ...
              <div style={{ marginTop: '0.4rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                {(s as any).isReal
                  ? <span style={{ fontSize: '0.65rem', color: '#10B981', fontWeight: 600 }}>✓ {s.change}</span>
                  : <span style={{ fontSize: '0.65rem', color: '#64748B', fontWeight: 500 }}>{s.change}</span>}
              </div>
            </motion.div>
          );
        })}
            ...
            {feedItems.map((f, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                  ...
                  <a href={`${BLOCK_EXPLORER}/address/0x1cd62cb08754a12fcc3427559e616a2898812d59`} target="_blank" rel="noopener noreferrer"
                    style={{ fontSize: '0.58rem', color: 'var(--text-dim)', fontFamily: 'JetBrains Mono,monospace', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.15rem' }}>
            ...
            {agentsWithYields.map((a, i) => (
              <motion.div key={i} whileHover={{ x: 2 }}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {a.apyBadge && <span style={{ fontSize: '0.52rem', padding: '0.1rem 0.375rem', borderRadius: 99, background: a.apyBadge.bg, color: a.apyBadge.color, fontWeight: 800 }}>{a.apyBadge.label}</span>}
                </div>
                <div style={{ textAlign: 'right' }}>
                  {yieldLoading ? (
                    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>Fetching…</div>
                  ) : (
                    <div>
                      <div style={{ fontSize: '1.2rem', fontWeight: 900, color: a.color, fontFamily: 'Outfit,sans-serif' }}>{a.apy !== null ? `${a.apy}%` : 'N/A'}</div>
                      {a.apySource && (
                        <div style={{ fontSize: '0.65rem', color: '#94A3B8', marginTop: 6 }}>
                          <span style={{ display: 'inline-block', marginRight: 8 }}>{a.apySource}</span>
                          {a.apySourceLink && (
                            <a href={a.apySourceLink} target="_blank" rel="noopener noreferrer" style={{ color: '#60A5FA', textDecoration: 'underline', fontWeight: 700 }}>Verify →</a>
                          )}
                          {a.apyLastUpdated && (
                            <div style={{ fontSize: '0.62rem', color: '#64748B', marginTop: 4 }}>Updated {timeAgo(a.apyLastUpdated)}</div>
                          )}
                        </div>
                      )}
                    </div>
